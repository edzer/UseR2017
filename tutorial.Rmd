...
During the [UseR! 2016 keynote](https://channel9.msdn.com/Events/useR-international-R-User-conference/useR2016/Towards-a-grammar-of-interactive-graphics) of Hadley, list-columns have become tidy. One of the arguments for this was exactly this: polygons with holes are hard to represent in simple `data.frame`s. Other cases are: nested `data.frame`s, or columns that contain, for each record, a model object e.g. obtained from `lm`.
...
* all functions/methods start with `st_`, use tab completion to search
* `read_sf` is an alias for `st_read` with tidyverse defaults: silent, `stringAsFactors = FALSE`
...
Coordinates of geospatial data need a reference system:
...
plot(st_buffer(u, 0.1), border = 'blue', add = TRUE)
par(opar)
```

### higher-level operations using these:

* `aggregate`, `summarise`: unions (by default) grouped geometries
* `st_interpolate_aw` for area-weighted interpolation
...
When _both_ `sf` and `dplyr` are loaded, a number of `dplyr` generic methods become available for `sf` objects. They are:
```{r}
m1 = attr(methods(class = "sf"), "info")$generic
suppressPackageStartupMessages(library(dplyr))
m2 = attr(methods(class = "sf"), "info")$generic
noquote(grep("_$", setdiff(m2, m1), invert = TRUE, value = TRUE))
```
...
nc1 <- nc %>% mutate(area = as.numeric(st_area(.)), longitude = cent[,1]) %>%
...
* support by [mapview](https://cran.r-project.org/package=mapview), [tmap](https://cran.r-project.org/package=tmap), [mapedit](https://github.com/r-spatial/mapedit)
* binary geom ops: fast, low memory footprint; flexible `st_join`
* (c)lean Rcpp interface to external dependencies GDAL/GEOS/Proj.4
* fast WKB I/O, written in C++

Still to do (no promises):
